<<<<<<<< HEAD:frontend/src/modules/detail/detail.js
...
import { GET_GARAGES, GET_SPECIFIC_CAR  } from '../../conf/main';
...
const Detail = () => {
  ...
  const FetchSpecificCar = async () => {
    try {
      const response = await client.query({ query: GET_SPECIFIC_CAR, variables: { id } });
      console.log('🚀 API Response:', response.data);
      
      const car = response.data.garage;
...
export default Detail;
>>>>>>>> origin/develop:frontend/src/pages/buy/buycar/detail.js
