...
export async function uploadAtEntryCreationAction(formData: any, image: File | null, selectedModelId: string, jwtSell: string) {
  try {
    // Handle image upload
    ...
    if (image) {
      ...
      const uploadResponse = await fetch(`${process.env.REACT_APP_BASE_URL}/api/upload`, {
        method: "POST",
        body: imageData,
      });
    ...
    const response = await fetch(`${process.env.REACT_APP_BASE_URL}/api/garages`, {
      method: "POST",
      headers: {
